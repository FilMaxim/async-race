...
class View {
    ...
    createInputUpdate(name = '', color = 'undefined') {
        ...
        this.inputBTNUpdate = this.createElement('button', ['btn', 'btn-update']) as HTMLButtonElement;
    ...
    createInputCreate() {
        ...
        const inputBTN = this.createElement('button', ['btn', 'btn-create']) as HTMLButtonElement;
    ...
    createRaceResetGenerateBTN() {
        ...
        this.btnRace = this.createElement('button', ['btn', 'btn-Race', 'btn-dark']);
        this.btnRace.textContent = 'Race';

        this.btnReset = this.createElement('button', ['btn', 'btn-Reset', 'btn-warning']);
        this.btnReset.textContent = 'Reset';

        this.btnGenerateCars = this.createElement('button', ['btn', 'btn-GenerateCars', 'btn-warning']);
    ...
    createOneCar(carData: CarData) {
        ...
        const carChangeRemoveBtns = this.createElement('div', ['car-change-remove-btns']);
        const btnChange = this.createElement('button', ['btn', 'btn-change']);
        btnChange.textContent = 'Change';
        const btnRemove = this.createElement('button', ['btn', 'btn-remove']);
        btnRemove.textContent = 'Remove';
        carChangeRemoveBtns.append(btnChange, btnRemove);
        const carStartResetBtns = this.createElement('div', ['car-start-reset-btns']);
        const moveCarBtnStart = this.createElement('div', ['btn', 'btn-start']);
        moveCarBtnStart.textContent = 'Start';
        const moveCarBtnReset = this.createElement('div', ['btn', 'btn-reset']);
        moveCarBtnReset.textContent = 'Reset';
        carStartResetBtns.append(moveCarBtnStart, moveCarBtnReset);
        carToolBar.append(carChangeRemoveBtns, carStartResetBtns);

        const wrapNameMove = this.createElement('div', ['wrap-name-move']);
        ...
        wrapNameMove.append(carName, moveCar);
        this.car.append(carToolBar, wrapNameMove);
    ...
    bindRemoveCar(handler: { (id: number): void; (arg0: number): void }) {
        this.carsWrapper.addEventListener('click', (event) => {
            ...
            if (targ.classList.contains('btn-remove')) {
    ...
    bindUpdateCar(
        ...
        this.carsWrapper.addEventListener('click', (event) => {
            const targ = event.target as HTMLElement;
            if (targ.classList.contains('btn-change')) {
                const elementCar = targ.closest('.car');
    ...
    //клик по кнопке Start и Reset
    bindStartResetCar(handler: {
        (id: number): Promise<((response: { json: () => unknown }) => unknown) | undefined>;
        (arg0: number): void;
    }) {
        let id: number;
        this.carsWrapper.addEventListener('click', (event) => {
